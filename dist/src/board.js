...
export class Board {
    width;
    height;
    cards; // двумерный массив карт
    playerCards; // карты под контролем игроков
    previousCards; // предыдущие карты игрока для правил 3-A/3-B
    watchers; // кто ждёт изменений (для watch)
    waitingForCard; // игроки, ждущие карту (правило 1-D)
    ...
    checkRep() {
        ...
        for (const [playerId, positions] of this.playerCards.entries()) {
            ...
            for (const pos of positions) {
                ...
                assert(card.controlledBy === playerId, `player ${playerId} claims control but card controlled by ${card.controlledBy}`);
        ...
        for (const [playerId, prev] of this.previousCards.entries()) {
            assert(prev.positions.length >= 1 && prev.positions.length <= 2, `player ${playerId} has invalid previousCards count`);
            if (prev.matched) {
