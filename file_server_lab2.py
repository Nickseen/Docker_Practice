#!/usr/bin/env python3
"""
Lab 2: Multithreaded HTTP File Server with Thread Pool
Features:
...
class ThreadPool:
    ...
    Workers continuously process tasks from a thread-safe queue.
    """
...
# ============================================================================
# HTTP FILE SERVER WITH THREAD POOL
# ============================================================================

class HTTPFileServer:
    """
    HTTP File Server with thread pool and advanced features:
    - Concurrent request handling
    - Request counter (naive and thread-safe versions)
    - Rate limiting by IP
    """
    ...
    def __init__(self, serve_directory, host='0.0.0.0', port=8080, 
        ...
        Args:
            ...
            simulate_work_delay: Delay in seconds to simulate work (0 = no delay)
            use_locks: Use locks for thread-safe counters (True = safe, False = race condition)
            enable_rate_limiting: Enable rate limiting by IP
            rate_limit: Max requests per second per IP
        """
        ...
        # Request counter (tracks requests per file)
        self.request_counter = defaultdict(int)
        self.counter_lock = threading.Lock()  # Lock for thread-safe counter
        
        # Rate limiting (tracks requests per IP)
        self.ip_requests = defaultdict(list)  # IP -> list of timestamps
    ...
    def handle_request(self, client_socket, client_address):
        """Handle a single HTTP request (runs in worker thread)."""
    ...
    def _increment_counter(self, path):
        """
        Increment request counter for a file.
        
        Demonstrates race condition when use_locks=False:
        - Multiple threads read the same value
        - Each increments independently
        - Some increments are lost!
        
        With use_locks=True, uses a lock to prevent race condition.
        """
        if self.use_locks:
            # THREAD-SAFE VERSION (correct)
    ...
    def serve_directory_listing(self, client_socket, dir_path, requested_path):
        """Serve directory listing with request counts."""
        try:
...
"""
    ...
    def send_response(self, client_socket, status_code, content_type, body):
        """Send HTTP response with text body."""
    ...
    def send_binary_response(self, client_socket, status_code, content_type, body_bytes):
        """Send HTTP response with binary body."""
        status_text = self.get_status_text(status_code)
        response_headers = f"HTTP/1.1 {status_code} {status_text}\r\n"
        response_headers += f"Content-Type: {content_type}\r\n"
